...
def detectar_y_describir(imagen, detector_tipo="SIFT"):
    ...
    if detector_tipo == "SIFT":
        ...
        print(
            f"Advertencia: Detector '{detector_tipo}' no reconocido. Usando SIFT por defecto."
        )
        detector = cv2.SIFT_create()
...
def unificar_dos_imagenes(img_base_color, img_to_warp_color, H):
    ...
    corners = np.float32(
        [[0, 0], [0, alto_warp], [ancho_warp, alto_warp], [ancho_warp, 0]]
    ).reshape(-1, 1, 2)
    ...
    T_translate = np.array(
        [[1, 0, tx_desplazamiento], [0, 1, ty_desplazamiento], [0, 0, 1]],
        dtype=np.float32,
    )
    ...
    img_unificada = cv2.warpPerspective(
        img_to_warp,
        H_final,
    ...
    ROI_y_start = ty_desplazamiento
    ROI_y_end = ty_desplazamiento + alto_base
    ROI_x_start = tx_desplazamiento
    ROI_x_end = tx_desplazamiento + ancho_base
    ...
    if roi_unificada.shape != img_base.shape:
        print(
            f"!!! DIAGNÓSTICO: Mismatch Dimensional entre ROI ({roi_unificada.shape}) e img_base ({img_base.shape}) !!!"
        )
        print(
